...
const SERVICE_CARDS_ROW2 = [
  ...
  },
  {
    ...
    label: "Payment",
    icon: Wallet,
    description: "Exchange rates, ATM locators, and secure digital payment partners.",
    to: "/services/payment",
  },
  {
    key: "emergency",
    ...
    icon: PhoneCall,
    description: "Direct lines to tourist police, embassies, and medical facilities.",
    to: "/emergency",
...
const SHORTCUTS = [
  { key: "restaurants-shortcut", label: "Restaurants", icon: Utensils, to: "/services/restaurants" },
  { key: "currency", label: "Currency", icon: RefreshCw, to: "/services/currency" },
  { key: "translation", label: "Translation", icon: Languages, to: "/services/translation" },
];
...
function ServiceCard({ label, icon: Icon, description, to, iconClass }) {
  return (
    <div className="flex h-full flex-col items-center rounded-2xl bg-[#EDE0C9] px-6 py-8 text-center">
      <Icon className={`h-8 w-8 ${iconClass || "text-[#846B20]"}`} strokeWidth={1.75} />
      <h3 className="mt-4 text-lg font-bold text-[#4F3B00]">{label}</h3>
      <p className="mt-2 text-sm leading-relaxed text-[#4F3B00]/85">{description}</p>
      <Link
        to={to}
...
export default function Services() {
  return (
    <div className="min-h-screen bg-[#FAF6ED] font-body">
      <SiteNavbar />

      <main className="mx-auto bg-[#FAF6ED] max-w-6xl px-6 pb-20 pt-10 sm:px-10">
        <h1 className="text-3xl font-bold text-black sm:text-4xl">Travel Services</h1>
        <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-[#4C4546]">
          Direct Access to Trusted Services. Everything you need for a seamless journey through
          Egypt, curated and verified.
        </p>
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-3">
          {SHORTCUTS.map(({ key, label, icon: Icon, to }) => (
            ...
            >
              ...
              <span className="mt-3 text-sm font-semibold text-[#2B2620]">{label}</span>
            </Link>
          ))}
        </div>
      </main>
